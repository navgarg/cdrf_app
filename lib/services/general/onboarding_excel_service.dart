...
class OnboardingExcelService {
  ...
  Future<void> exportOnboardingDataToExcel(List<Map<String, dynamic>> data) async {
    final excel = Excel.createExcel();
    final Sheet sheet = excel['Onboarding Data'];

    if (data.isNotEmpty) {
      // Add headers
      sheet.appendRow(data.first.keys.map((key) => TextCellValue(key)).toList());

      // Add data rows
      for (var row in data) {
        sheet.appendRow(row.values.map((value) => TextCellValue(value.toString())).toList());
      }
    }
    ...
    if (fileBytes != null) {
      ...
      final path = '${directory.path}/OnboardingData_${DateTime.now().millisecondsSinceEpoch}.xlsx';
      File(path)
